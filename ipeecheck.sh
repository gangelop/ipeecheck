...
| mailx -v -s "[IPSWITCHEE] Your ip address has changed" "$EMAIL"
    if $ENABLE_DESKTOP_NOTIFICATION;
      then
	if $REMOTE_DESKTOP_NOTIFICATION;
	  then
	    ssh $REMOTE_NOTIFICATON_IP "DISPLAY=$DESKTOP_NOTIFICATION_DISPLAY notify-send \"IpSwitchee\" \"Your new public IP is:\n$CURRENT_ADDR\""
	  else
	    DISPLAY=$DESKTOP_NOTIFICATION_DISPLAY notify-send "IpSwitchee" "Your new public IP is:\n$CURRENT_ADDR"
	fi
    fi
fi
